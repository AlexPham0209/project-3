...
export default function SpeakingTable({name, credits}) {
    ...
    let rows = Array.from({ length: numberOfCourses }, (_, i) => 
        (i === 0 ? <CourseRow id={1} prefix={'SPK'} number={'208/230'} description={`Speaking${1}` }></CourseRow>
            : <CourseRow id={i + 1} description={`Speaking${i + 1}`}></CourseRow> 
        )
    );

    return (
    <div className="course-table">
        <div className="title">Speaking Emphasis for {name}</div>
